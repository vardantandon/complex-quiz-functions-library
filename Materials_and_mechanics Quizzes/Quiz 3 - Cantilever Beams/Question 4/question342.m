...
text(Le-20, -2, strcat('Question ID 2 - #', ID),'fontsize',8,'fontweight','bold');
...
%Question string joining
    ...
    '<p>a) Calculate the magnitude of the reaction force R and the moment M to the nearest whole number.',...
    '<p></p><p>R:&nbsp', numString_r,'N</p>', ...
    '<p>M:&nbsp', numString_m, 'Nm</p>', ...
    '<br>b) Calculate and draw the Shear Force Diagram (SFD) on paper and identify the following values (to the nearest whole number):</p>', ...
    '<p>Shear force just to the right of A (i.e. x=0cm):&nbsp', numString_SFA, 'N</p>', ...
    '<p>Shear force just to the right of B (i.e. x=', num2str(round(Pb*100)),'cm):&nbsp', numString_SFB, 'N</p>', ...
    '<p>Shear force just to the right of C (i.e. x=', num2str(round(Pc*100)),'cm):&nbsp', numString_SFC, 'N</p>', ...
    '<p>Shear force just to the right of D (i.e. x=', num2str(round(Pd*100)),'cm):&nbsp', numString_SFD, 'N</p>', ...
    '<p>Shear force just to the right of E (i.e. x=', num2str(round(Pe*100)),'cm):&nbsp', numString_SFE, 'N</p>', ...
    '<p>Shear force just to the right of F (i.e. x=', num2str(round(L*100)),'cm):&nbsp', numString_SFF, 'N</p>', ...
    ...
    '<br>c) Calculate and draw the Bending Moment Diagram (BMD) on paper and identify the following values (to the nearest whole number):', ...
    '<p>Bending moment at A:&nbsp', numString_BMA, 'Nm</p>', ...
    '<p>Bending moment at B:&nbsp', numString_BMB, 'Nm</p>', ...
    '<p>Bending moment at C:&nbsp', numString_BMC, 'Nm</p>', ...
    '<p>Bending moment at D:&nbsp', numString_BMD, 'Nm</p>', ...
    '<p>Bending moment at E:&nbsp', numString_BME, 'Nm</p>', ...
    '<p>Bending moment at F:&nbsp', numString_BMF, 'Nm</p>', ...
    ...
    generalfeedbackString=strcat('<p>Below is the equivalent free body diagram for this system.</p><p>!Note - for the sake of clarity distances are not to scale on this diagram!',...
        '<img src="@@PLUGINFILE@@/image2.png" style="width:100%%; height:auto;">',...
        '<p>The SFD can be defined by the following co-ordinates:</p>', ...
        ...
        '<p></p><p></p><p>The BMD is the integral of the SFD, and looks like this in this case:</p><p></p>', ...
   ...
   %Complete question XML code generation
    xmlCode = question_cloze(i, ...
        ...
        '<p>Remember to produce an equivalent free body diagram first, and then apply the following equations:</p><p>$$\\small\\sum$$ M<sub>z</sub>=0</p><p>$$\\small\\sum$$ F<sub>y</sub>=0</p><p>$$\\small\\sum$$ F<sub>x</sub>=0</p>.', ...
        '<p>When producing SFDs and BMDs for cantilever beams, neither plot will start at the origin, but both will end on the x-axis.</p><p>The bending moment can be calculated by adding all of the bending moments to the left or right of any discrete point along the beam.</p><p>A point force will produce a vertical line on a SFD.</p><p>A UDL will always produce a line with a slop an a SFD, and a curve on a BMD.</p>', ...
        plotString1, ...
        plotString2, ...
        plotString3, ...
        plotString4);
