...
file=fopen('cantilever5-2.xml','w');
...
IDString=strcat('Cantilever Beams. Level 5.Question ID # 2 - ', ID);  
% Start Image
...
%Plotting and labelling the reaction force
...
plot([P2,P2],[0.1,0.5],'Linestyle','--','linewidth',2,'Color','black');


U2_label=strcat(num2str(U2),'N');

text(P2-Le/30,0.6,U2_label,'fontsize',14,'fontweight','bold');
...
text(P4-Le/30,0.5,U3_label,'fontsize',14,'fontweight','bold');
...
plot([P4,P4],[0.1,-0.8],'Linestyle','--','linewidth',2,'Color','black');

% ==========
% % Plotting arrow for U4
% ==========

arrow([P5,0.2],[P5,0.1],'LineWidth',2); %plots a point load at the start of the ist distributed load
plot([P5,P5],[0.1,0.5],'Linestyle','--','linewidth',2,'Color','black');

U4_label=strcat(num2str(U4),'N');

text(P5-Le/30,0.6,U4_label,'fontsize',14,'fontweight','bold');

b1=a1; %Starting point of measured position
b2=P5; %End point of measured position
...
plot([P5,P5],[0.1,-1.1],'Linestyle','--','linewidth',2,'Color','black');

% ==========
% % Plotting arrow for weight fo beam
% ==========

arrow([P3,-0.4],[P3,-0.5],'LineWidth',2); %plots a point load at the start of the ist distributed load
plot([P3,P3],[-0.1,-0.5],'Linestyle','--','linewidth',2,'Color','black');

U5_label=strcat(num2str(W_beam),'N');

text(P3-Le/15,0,U5_label,'fontsize',14,'fontweight','bold');
...
b3=(b1+b2)/2; %Midpoint
e=0.9;
arrow([b3,e],[b1,e],'LineWidth',2); %Draws l.h.s. of arrow
arrow([b3,e],[b2,e],'LineWidth',2); %Draws r.h.s. of arrow

P5_label=strcat(num2str(L/2),'m');

b4=b1+(b1+b2)*2/3;

text(b4*0.6,1.00,P5_label,'fontsize',14,'fontweight','bold');

plot([P3,P3],[0.9,0.1],'Linestyle','--','linewidth',2,'Color','black');
...
%Plotting the SFD
figure(3)
plot([0,Pb],[SFA,(SFB+U1)],'Color','red','linewidth',3);

hold on
plot([Pb,Pb],[(SFB+U1),SFB],'Color','red','linewidth',3);
plot([Pb,Pc],[SFB,SFC],'Color','red','linewidth',3);
plot([Pc,Pd],[SFC,SFD],'Color','red','linewidth',3);
...
plot([Pb,Pb],[0,SFA],'Linestyle','--','linewidth',1,'Color','black');
plot([Pc,Pc],[0,SFA],'Linestyle','--','linewidth',1,'Color','black');
plot([Pd,Pd],[0,SFA],'Linestyle','--','linewidth',1,'Color','black');
plot([Pe,Pe],[0,SFA],'Linestyle','--','linewidth',1,'Color','black');
...
% Plotting section 2/5

m2=(SFC-SFB)/(Pc-Pb);
c2=SFB-(m2*Pb);
X2=(Pb:0.01:Pc);

a2=BMC-(m2*Pc.^2/2+c2*Pc);

Y2=(m2.*(X2.^2)/2 + c2.*X2 +a2);

plot(X2,Y2,'linewidth',3,'Color','red');

% Plotting section 3/5

m3=(SFD-SFC)/(Pd-Pc);
c3=SFC-(m3*Pc);
X3=(Pc:0.01:Pd);

a3=BMD-(m3*Pd.^2/2+c3*Pd);

Y3=(m3.*(X3.^2)/2 + c3.*X3 +a3);

plot(X3,Y3,'linewidth',3,'Color','red');

% Plotting section 4/5

m4=(SFE+U3-SFD)/(Pe-Pd);
c4=SFD-(m4*Pd);
X4=(Pd:0.01:Pe);

a4=BMD-(m4*Pd.^2/2+c4*Pd);

Y4=(m4.*(X4.^2)/2 + c4.*X4 +a4);

plot(X4,Y4,'linewidth',3,'Color','red');

% Plotting section 5/5

m5=(SFF-SFE)/(L-Pe);
c5=SFE-(m5*Pe);
X5=(Pe:0.01:L);

a5=BMF-(m5*L.^2/2+c5*L);

Y5=(m5.*(X5.^2)/2 + c5.*X5 +a5);

plot(X5,Y5,'linewidth',3,'Color','red');

plot([Pb,Pb],[0,BMA],'Linestyle','--','linewidth',1,'Color','black');
plot([Pc,Pc],[0,BMA],'Linestyle','--','linewidth',1,'Color','black');
plot([Pd,Pd],[0,BMA],'Linestyle','--','linewidth',1,'Color','black');
plot([Pe,Pe],[0,BMA],'Linestyle','--','linewidth',1,'Color','black');
...
%Embedded components string generation
    ...
    numString_SFD_A_B = cloze_mcq('vertical', 3, 'Either a horizontal or vertical line', 'Incorrect', 'A combination of horizontal and vertical lines', 'Incorrect', 'A combination of a line with a slope and a straight line','Good job!', 'A curve' ,'Incorrect');
    ...
    numString_SFD_D_E = cloze_mcq('vertical', 3, 'Either a horizontal or vertical line', 'Incorrect', 'A combination of horizontal and vertical lines', 'Incorrect', 'A combination of a line with a slope and a straight line','Good job!', 'A curve' ,'Incorrect');
    ...
    numString_BMD_A_B = cloze_mcq('vertical', 3, 'Either a horizontal or vertical line', 'Incorrect', 'A combination of horizontal and vertical lines', 'Incorrect', 'A combination of a line with a slope and a straight line','Good job!', 'A curve' ,'Incorrect');
    ...
    numString_BMD_D_E = cloze_mcq('vertical', 3, 'Either a horizontal or vertical line', 'Incorrect', 'A combination of horizontal and vertical lines', 'Incorrect', 'A combination of a line with a slope and a straight line','Good job!', 'A curve' ,'Incorrect');
...
%Question string joining
    ...
    '<p>The beam itself has a weight of 1,500 N per m.</p>',...
    ...
    '<p><b>A)</b></p><p>Calculate the magnitude of the reaction force R and the moment M to the nearest whole number.</p>',...
    ...
    '<br><p><b>B)</b></p><p>Calculate and draw the Shear Force Diagram (SFD) on paper and identify the following values (to the nearest whole number):</p>', ...
    ...
    '<br><p><b>C)</b></p><p>Calculate and draw the Bending Moment Diagram (BMD) on paper and identify the following values (to the nearest whole number):', ...
    ...
    generalfeedbackString=strcat('<p><b>PART A</b></p><p>Below is the equivalent free body diagram for this system.</p><p>!Note - for the sake of clarity distances are not to scale on this diagram!</p>',...
        ...
        '<p>Don''t forget to include the weight of the beam in your calculations, which is modelled as a UDL.<p>',...
        ...
        '<p>R - &nbsp',num2str(W3),' - &nbsp',num2str(W),' - &nbsp',num2str(L*1500),' - &nbsp',num2str(W4),' - &nbsp',num2str(W2),' = 0</p>',...
        '<p>Therefore, R = &nbsp',num2str(R),'N.</p>',...
        '<p>The moment created by the wall must balance the moment created by the five point loads:</p>', ...
        '<p>M - (',num2str(Pb),'m x &nbsp',num2str(W3),'N) - (',num2str(round(AA*100)/100),'m x &nbsp',num2str(W),'N) - (',num2str(round(L/2*100)/100),'m x &nbsp',num2str(L*1500),'N)  - (',num2str(round(Pe*100)/100),'m x &nbsp',num2str(W4),'N)  - (',num2str(round(BB*100)/100),'m x &nbsp',num2str(W2),'N)  = 0</p>', ...
        '<p>Therefore, M = &nbsp',num2str(round(M)),'Nm.</p>',...
        '<p><b>PART B</b></p>In this case the SFD can be defined by the following co-ordinates:', ...    
        '<p>(0,',num2str(SFA),'), (',num2str(Pb),',',num2str(SFB+U1),'), (',num2str(Pb),',',num2str(SFB),'), (',num2str(Pc),',',num2str(SFC),'), (',num2str(Pd),',',num2str(SFD),'), (',num2str(round(Pe*100)/100),',',num2str(round(SFE+U3)),'),  (',num2str(round(Pe*100)/100),',',num2str(round(SFE)),'),  (',num2str(L),', 0)</p>', ...
        ...
        '<p>The BM at point B, considering forces to the left = &nbsp',num2str(round(BMA)),'Nm + (',num2str(Pb),'m x &nbsp', num2str(R),'N) - (',num2str(Pb/2),'m x &nbsp', num2str(Pb*1500),'N) = &nbsp',num2str(round(BMB)),'Nm.',...
        '<p>The BM at point C, considering forces to the left = &nbsp',num2str(round(BMA)),'Nm + (',num2str(Pc),'m x &nbsp', num2str(R),'N) - (',num2str(Pc-Pb),'m x &nbsp', num2str(W3),'N) - (',num2str((Pc-Pb)/2),'m x &nbsp', num2str(W),'N) - (',num2str(Pc/2),'m x &nbsp', num2str(Pc*1500),'N)  = &nbsp',num2str(round(BMC)),'Nm.',...
        '<p>The BM at point D, considering forces to the right = - (',num2str(round((Pe-Pd)*100)/100),'m x &nbsp', num2str(W4),'N) - (',num2str(BB-Pd),'m x &nbsp', num2str(W2),'N) - (',num2str((L-Pd)/2),'m x &nbsp', num2str((L-Pd)*1500),'N) = &nbsp',num2str(round(BMD)),'Nm.',...
        '<p>The BM at point E, considering forces to the right = - (',num2str(round(((L-Pe)/2)*100)/100),'m x &nbsp', num2str(round((L-Pe)/(L-Pd)*W2*100)/100),'N) - (',num2str((L-Pe)/2),'m x &nbsp', num2str((L-Pe)*1500),'N) = &nbsp',num2str(round(BME)),'Nm.',...
   ...
   %Complete question XML code generation
    xmlCode = question_cloze(i, ...
        ...
        '<p><b>HINT</b></p><p>Remember to produce an equivalent free body diagram first, and then apply the following equations:</p><p>$$\\small\\sum$$ M<sub>z</sub>=0</p><p>$$\\small\\sum$$ F<sub>y</sub>=0</p><p>$$\\small\\sum$$ F<sub>x</sub>=0</p><p>Don''t forget to include the weight of the beam in your calcualtions.</p><p>When producing SFDs and BMDs for cantilever beams, neither plot will start at the origin, but both will end on the x-axis.</p><p>The bending moment can be calculated by adding all of the bending moments to the left or right of any discrete point along the beam.</p><p>A point force will produce a vertical line on a SFD.</p><p>A UDL will always produce a line with a slope an a SFD, and a curve on a BMD.</p>', ...
        '',...
        plotString1, ...
        plotString2, ...
        plotString3, ...
