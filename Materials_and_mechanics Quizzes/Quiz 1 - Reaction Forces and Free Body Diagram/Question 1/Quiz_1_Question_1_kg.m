...
file=fopen('ssb1_reaction_forceskg.xml','w');
...
w = randi([8,15]); ... generates a point load between 8 and 15 kg
...
r2 = round((((wkg*p)/l)*100))/100; ... calculates R2 to 2.d.p.

r1 = round(((wkg-r2)*100))/100; ... calculates R1 to 2.d.p.
...
IDString=strcat('Reaction Forces and Free Body Diagram - Kg. Level 1.Question ID #', ID);
...
plotString2 = generateImageString(gcf, 'image2');
 ...
 %Question string joining
    questionString = strcat('<br><img src="@@PLUGINFILE@@/image1.png" style="width:100%%; height:auto;"><br><p>A simply supported beam of length&nbsp;', num2str(l), ' cm is shown in the figure above. There are two supports, one at either end of the beam, hence reactions forces R1 and R2. There is a point mass of&nbsp;',num2str(w),' kg acting at a distance&nbsp;',num2str(p),' cm from point A.</p><p>You may assume a value of g of 10 m/s<sup>2</sup>.</p>', ...
    ...
    generalfeedbackString=strcat('When a system is in equilibrium the total forces and the total moment need to be equal to zero; otherwise the system will be moving or rotating, respectively.</p>',...
        ...
        '<p>$$\\small\\sum$$ M<sub>z</sub>=0 $$\\rightarrow$$ (', num2str(l/100), 'm x R2) - (', num2str(p/100), 'm x&nbsp;', num2str(wkg), 'N) = 0</p>',...
        '<p>Hence:</p>',...
        '<p>R2 =&nbsp', num2str(r2),'N.</p>',...
        '<br><p>In this case resolving vertical forces, we have:</p>',...
        '<p>$$\\small\\sum$$ F<sub>y</sub>=0 $$\\rightarrow$$ R1 + R2 -&nbsp', num2str(wkg),' = 0.</p>',...
        '<p>Hence:</p>',...
