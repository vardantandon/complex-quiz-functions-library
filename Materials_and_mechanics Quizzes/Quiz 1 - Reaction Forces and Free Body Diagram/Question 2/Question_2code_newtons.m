file=fopen('ssb2_reaction_forcesN.xml','w');
...
r2=round((((p1*w1)+(p2*w2))/l)*100)/100;

r1=round((w1+w2-r2)*100)/100;
...
IDString=strcat('Reaction Forces and Free Body Diagram - N. Level 2.Question ID #', ID);
...
plotString2 = generateImageString(gcf, 'image2');
 ...
 %Question string joining
    questionString = strcat('<br><img src="@@PLUGINFILE@@/image1.png" style="width:100%%; height:auto;"><br><p>A simply supported beam of length&nbsp;',num2str(l),' cm is shown in the figure above. There are two supports, one at either end of the beam, hence reactions forces R1 and R2. There are two forces of&nbsp;', num2str(w1),' N and&nbsp;',num2str(w2),' N, acting at distances&nbsp;',num2str(p1),' cm and&nbsp;',num2str(p2),' cm, respectively from A.</p><p>By resolving forces and taking moments, determine the value of reaction forces, R1 and R2.</p>', ...
    ...
    generalfeedbackString=strcat('When a system is in equilibrium the total forces and the total moment need to be equal to zero; otherwise the system will be moving or rotating, respectively.</p>',...
        ...
        '<p>$$\\small\\sum$$ M<sub>z</sub>=0 $$\\rightarrow$$ (', num2str(l/100), 'm x R2) - (', num2str(p1/100), 'm x&nbsp;', num2str(w1), 'N) - (', num2str(p2/100), 'm x&nbsp;', num2str(w2), 'N) = 0</p>',...
        '<p>Hence:</p>',...
        '<p>R2 =&nbsp', num2str(r2),'N.</p>',...
        '<br><p>In this case resolving vertical forces, we have:</p>',...
        '<p>$$\\small\\sum$$ F<sub>y</sub>=0 $$\\rightarrow$$ R1 + R2 -&nbsp', num2str(w1),' -&nbsp', num2str(w2),' = 0.</p>',...
        '<p>Hence:</p>',...
