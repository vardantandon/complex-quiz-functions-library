...
IDString=strcat('Second Moment of Area. Level 3.Question ID 2#', ID); 
...
text(W+5, -20, strcat('Question ID 2#', ID),'fontsize',8,'fontweight','bold');
...
%Question string joining
    ...
    '<p><b>PART A</b></p><p>As this beam is symmetrical about the x-axis, the centre of mass with respect to the y-axis is simply the midpoint on the y-plane.</p>',...
    '<p>As this beam is not symmetrical about the y-axis, the centre of mass with respect to the x-axis can be calculated as follows:</p><p>((Area<sub>1</sub> x centroid<sub>x1</sub>) + (Area<sub>2</sub> x centroid<sub>x2</sub>) + (Area<sub>3</sub> x centroid<sub>x3</sub>)) / Area<sub>total</sub>',...
    '<p>Where centroid<sub>x1</sub>, centroid<sub>x2</sub>, and centroid<sub>x3</sub> are the distances from the centre of each rectangle to the furthest left-hand edge of the I-beam, respectively.</p>',...
    ...
    '<p><b>PART B</b></p><p>As the beam is symmetrical about the x-axis, and therefore all of the three component rectangles share the same neutral axis in the x direction, use the following formula to calculate the second moment of area with respect to the x-axis (as per the principles of question 2 of this quiz):</p>',...
    '<p>I<sub>x</sub> = I<sub>x1</sub> + I<sub>x2</sub> + I<sub>x3</sub></p>',...
    '<p></p><p>As the shape is not symmetrical about the y-axis, you will need to use the parallel axis theorem to calculate the second moment of area with respect to y, as follows:</p>',...
    '<p>I<sub>Total</sub> = I<sub>YY1</sub>+ I<sub>YY2</sub>+ I<sub>YY3</sub> = $$\\small\\sum$$[I<sub>NAi</sub> + (A<sub>i</sub> x d<sub>i</sub><sup>2</sup>)].</p>',....
    ...
    '<p>d<sub>i</sub> = the difference between the x-centroid of the respective rectangle and the x-centroid of the whole I-beam.</p>',...
    '<p>Therefore:</p>',...
