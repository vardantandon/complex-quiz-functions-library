...
% Generates image for level 3 Warren Truss problems
% question name - Warren Truss level 3
...
IDString=strcat('Warren Truss. Level 3.Question ID #', ID);  
...
rectangle('Position',[c+2*d,e,d,d],'Curvature',[1,1],'LineWidth',1.5,'FaceColor','y');
rectangle('Position',[c+3*d,e,d,d],'Curvature',[1,1],'LineWidth',1.5);
text (c+0.1, e-d/2 ,'Level 3','FontWeight', 'bold', 'fontsize',15);


if Fv1 <0
    text (0.75, 2.1, '\downarrow','color', 'black', 'fontsize',50)
else
    text (0.75, 2.1, '\uparrow','color', 'black', 'fontsize',50)     
end
text (0.9,2.5, strcat('F_{V1}= ',num2str(abs(Fv1)),' N'), 'fontsize', 18)

if Fv2 <0
    text (2.75, 2.1, '\downarrow','color', 'black', 'fontsize',50)
else
...
Re = (Fv1/tand(angle1)  + Fv3*(1/tand(angle1)+1/tand(angle2))  + Fv2*(1/tand(angle1)+2/tand(angle2)))  / (-2/tand(angle1)-2/tand(angle2)); %Pilar
Ra = -Fv1-Fv2-Fv3-Re;

% resolving forces
F1 = -Ra/sind(angle1);                            % Joint A, F_AB. Correct!
F3 = -F1*cosd(angle1);                               % Joint A, F_AC. Correct!

F2 = (Fv1-F1*cosd(angle2))/cosd(angle1);  % Joint B, F_BC
F4 = F1*sind(angle2)-F2*sind(angle1);          % Joint B, F_BD

F7 = -Re/sind(angle1);                             % Joint E, F_DE. Correct!
F6 = -F7*cosd(angle1);    % Joint C, F_CE. Correct!


F5 = (Fv2-F7*cosd(angle2))/cosd(angle1);   % Joint D, F_CD. Correct!
...
F7A='negative';
    ...
    numString_AB = cloze_numerical(round(F1), 0.025*F1, 'Good job!', 'Incorrect');
    numString_BC = cloze_numerical(round(F2), 0.025*F2, 'Good job!', 'Incorrect');
    numString_AC= cloze_numerical(round(F3), 0.025*F3, 'Good job!', 'Incorrect');
    numString_BD= cloze_numerical(round(F4), 0.025*F4, 'Good job!', 'Incorrect');
    numString_CD= cloze_numerical(round(F5), 0.025*F5, 'Good job!', 'Incorrect');
    numString_CE= cloze_numerical(round(F6), 0.025*F6, 'Good job!', 'Incorrect');
    numString_DE= cloze_numerical(round(F7), 0.025*F7, 'Good job!', 'Incorrect');

    

%Question string joining
    questionString = strcat('<br><img src="@@PLUGINFILE@@/image1.png" style="width:100%%; height:auto;">', ...
    '<br><p></p>Consider the system of triangles in the truss problem as shown above, where $$\\small\\theta$$  = &nbsp',num2str(angle1),'<sup>o</sup> and $$\\small\\phi$$ = &nbsp',num2str(angle2), '<sup>o</sup>', ...
    'and the forces applied on the system are F<sub>V1</sub> = &nbsp', num2str(abs(Fv1)),'N, F<sub>V2</sub> = &nbsp', num2str(abs(Fv2)),'N, and F<sub>V3</sub> = &nbsp', num2str(abs(Fv3)),'N.</p>',...
    ...
    '<p><b>PART A</b></p><p>R<sub>A_x</sub> must equal zero, as there are no other horizontal forces to consider, and the system is in equilibirum.</p>',...
        ...
        '<p>(R<sub>E</sub> x ((2xAG)+(2xGC)) - (F<sub>V1</sub> x AG) - (F<sub>V3</sub> x (AG+GC)) - (F<sub>V2</sub> x (AG+(2xGC)))  = 0</p>',...
        '<p>We can then substitute in the distances AG and GC (as calculated above), the values of F<sub>V1</sub>, F<sub>V2</sub>, F<sub>V3</sub>, $$\\small\\theta$$ and $$\\small\\phi$$ as given in the question, and divide the whole expression by h.</p>',...
        '<p>This gives a value of R<sub>E</sub> = &nbsp', num2str(round(Re)), 'N.</p>',...
        '<p>For the whole system to be in equilibrium: R<sub>A_y</sub> + R<sub>E</sub> - F<sub>V1</sub> - F<sub>V2</sub> - F<sub>V3</sub> = 0.</p>',...
    ...
    '<p>-(F<sub>DE</sub> x cos$$\\small\\phi$$) - (F<sub>CD</sub> x cos$$\\small\\theta$$) - F<sub>V2</sub> = 0</p>',...
    ...
    '<p>-(F<sub>CD</sub> x sin$$\\small\\theta$$) + (F<sub>DE</sub> x sin$$\\small\\phi$$) - F<sub>BD</sub> = 0</p>',...
    ...
    '<p>-(F<sub>AB</sub> x cos$$\\small\\phi$$) - (F<sub>BC</sub> x cos$$\\small\\theta$$) - F<sub>V1</sub> = 0</p>',...
