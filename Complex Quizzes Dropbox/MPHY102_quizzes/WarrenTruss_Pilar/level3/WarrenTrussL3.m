...
comb1=fopen('Warren Truss L3.xml','w'); 
...
title('Warren Truss level 3','FontSize',20, 'FontWeight','bold')
...
warrentruss=strcat('WarrenTrussL3_',num2str(i),'.jpg');
...
fprintf(comb1,'\n%s','<name>');
fprintf(comb1,'\n<text>Warren Truss %d Level 3</text>',i);
fprintf(comb1,'\n%s','</name>');

fprintf(comb1,'\n%s','<questiontext format="html">');
fprintf(comb1,'\n%s','<text>');
fprintf(comb1,'\n%s','<![CDATA[<p><img style="width: 625px; heght: 500px; margin-right: auto; margin-left: auto; display: block;"');
fprintf(comb1,'src="http://www.ucl.ac.uk/~rmapdpg/MoodleQuiz/WarrenTruss/Level_3/WarrenTrussL3_%d.jpg"',i);
fprintf(comb1,'%s','alt="truss" width="30 height="30"/></p>');

fprintf(comb1,'\n<p style="text-align: left;">Consider the system of right angle triangles in the Truss problem as shown above,');
...
fprintf(comb1,'%s','Draw a free body force diagram for each joints and using the method of joints, calculate');
...
Re(i) = (Fv1(i)/tand(angle1(i))  + Fv3(i)*(1/tand(angle1(i))+1/tand(angle2(i)))  + Fv2(i)*(1/tand(angle1(i))+2/tand(angle2(i))))  / (-2/tand(angle1(i))-2/tand(angle2(i))); %Pilar
...
% resolving forces
F1(i) = -Ra(i)/sind(angle1(i));                            % Joint A, F_AB. Correct!
F3(i) = -F1(i)*cosd(angle1(i));                               % Joint A, F_AC. Correct!

F2(i) = (Fv1(i)-F1(i)*cosd(angle2(i)))/cosd(angle1(i));  % Joint B, F_BC
F4(i) = F1(i)*sind(angle2(i))-F2(i)*sind(angle1(i));          % Joint B, F_BD

F7(i) = -Re(i)/sind(angle1(i));                             % Joint E, F_DE. Correct!
F6(i) = -F7(i)*cosd(angle1(i));    % Joint C, F_CE. Correct!


F5(i) = (Fv2(i)-F7(i)*cosd(angle2(i)))/cosd(angle1(i));   % Joint D, F_CD. Correct!
...
fprintf(comb1,'\n%s','<p style="text-align: center;" >');
fprintf(comb1,'F<sub>BD</sub> = {1:NUMERICAL:=%3.0f:1#Good job!~*#Sorry. Try again.} N</p>',F4(i));
fprintf(comb1,'\n%s','<p style="text-align: center;" >');
fprintf(comb1,'F<sub>CD</sub> = {1:NUMERICAL:=%3.0f:1#Good job!~*#Sorry. Try again.} N</p>',F5(i));
fprintf(comb1,'\n%s','<p style="text-align: center;" >');
fprintf(comb1,'F<sub>CE</sub> = {1:NUMERICAL:=%3.0f:1#Good job!~*#Sorry. Try again.} N</p>',F6(i));
fprintf(comb1,'\n%s','<p style="text-align: center;" >');
fprintf(comb1,'F<sub>DE</sub> = {1:NUMERICAL:=%3.0f:1#Good job!~*#Sorry. Try again.} N</p>',F7(i));
...
fprintf(comb1,'\n%s','<![CDATA[<p><img style="width: 625px; heght: 500px; margin-right: auto; margin-left: auto; display: block;"');
fprintf(comb1,'src="http://www.ucl.ac.uk/~rmapdpg/MoodleQuiz/WarrenTruss/Level_3/solution.jpg"');
fprintf(comb1,'%s','alt="truss" width="30 height="30"/></p>');
