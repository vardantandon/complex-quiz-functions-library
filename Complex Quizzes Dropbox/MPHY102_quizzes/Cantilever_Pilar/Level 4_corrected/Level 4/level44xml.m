...
imgname=strcat('level4_',num2str(i),'.jpg');
...
M=U1*Pb+U2*((Pc+Pb)/2)+U3*Pe+U4*((L+Pd)/2);
...
SF=plot([0,Pb],[SFA,SFA],'Color','red','linewidth',3);
...
plot([Pd,Pe],[SFC,SFD],'Color','red','linewidth',3);
...
shearforce=strcat('shearforce_level4_',num2str(i),'.jpg');
...
% %1st segment
% 
% BM=plot([0,Pb],[BMA,BMB],'linewidth',3,'Color','red');
% 
% hold on
% 
% %2nd segment
...
fprintf(comb1,'\n%s','<p>3) C and D {1:MULTICHOICE:~=Either a horizontal or vertical line.~A combination of horizontal and vertical lines.~A combination of a slope and a straight line.~A curve}</p>');
...
fprintf(comb1,'src="http://www.ucl.ac.uk/~wmapsfl/moodle/cantilever4/shearforce_level4_%d.jpg"',i);
fprintf(comb1,'%s','alt="Beamplot" width="30 height="30"/></p>');
